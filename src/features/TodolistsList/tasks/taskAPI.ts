...
export const taskAPI = {
  ...
  createTask(args: TCreateTaskArgs) {
    return instance.post<ResponseType<{ item: TTask }>>(`todo-lists/${args.todolistId}/tasks`, {
      title: args.taskTitle,
    });
  },
  updateTask(args: TUpdateTaskArgs) {
    return instance.put<ResponseType<{ item: TTask }>>(
...
type TGetTasksResponse = { error: string | null; totalCount: number; items: TTask[] };
